        ...
        <Flex m={4} p={8} direction="column" borderRadius="16px" boxShadow="2px 3px 10px 3px #888888" overflow="auto">
          ...
          <Flex direction="column" width="35vw" height="50vh" align="center" justify="flex-start" overflowY="scroll">
            <Table className="table" size="sm" variant='striped' colorScheme="whatsapp">
              <Thead>
                <Tr>
                  <Th textAlign="left">Product Name</Th>
                  <Th textAlign="left">Unit Price</Th>
                  <Th textAlign="left">Order Qty</Th>
                  <Th textAlign="left">Sub Total</Th>
                </Tr>
              </Thead>
              <Tbody>
                {Object.keys(cartItems).length > 0 ? <>{Object.keys(cartItems).map((id, index) => {
                  ...
                  return <Tr key={index + 1000}>
                    <Td textAlign="left">{cartItem.name}</Td>
                    <Td textAlign="left">${cartItem.unitPrice.toFixed(2)}</Td>
                    <Td textAlign="left">{cartItem.orderQty}</Td>
                    <Td textAlign="left">${(cartItem.unitPrice * cartItem.orderQty).toFixed(2)}</Td>
                  </Tr>
                })}
                  <Tr>
                    <Td colSpan={2}></Td><Td colSpan={1} textAlign="right">Net Total</Td><Td colSpan={1} textAlign="left">${Object.keys(cartItems).reduce((acc, id) => (acc + cartItems[id].unitPrice * cartItems[id].orderQty), 0).toFixed(2)}</Td>
                  </Tr>
                  <Tr>
                    <Td colSpan={2}></Td><Td colSpan={1} textAlign="right">GST</Td><Td colSpan={1} textAlign="left">${(Object.keys(cartItems).reduce((acc, id) => (acc + cartItems[id].unitPrice * cartItems[id].orderQty), 0)*0.08).toFixed(2)}</Td>
                  </Tr>
                  <Tr>
                    <Td colSpan={2}></Td><Td colSpan={1} textAlign="right">Total with GST</Td><Td colSpan={1} textAlign="left">${(Object.keys(cartItems).reduce((acc, id) => (acc + cartItems[id].unitPrice * cartItems[id].orderQty), 0)*1.08).toFixed(2)}</Td>
                  </Tr>
                  <Tr>
                    <Td colSpan={4} textAlign="center"><Button onClick={(ev) => handlePlaceOrder(ev)}> Place Order {loading2 && <Spinner size='sm' />}</Button></Td>
                  </Tr>
                  </>
                  :
